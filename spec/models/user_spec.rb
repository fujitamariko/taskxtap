...
RSpec.describe 'Userモデル機能', type: :model do
  describe 'バリデーションのテスト' do
    ...
    context '名前が20文字以上の場合' do
      it 'バリデーションに引っかかる' do
        user = FactoryBot.build(:user, name: 'n'*21)
        expect(user).not_to be_valid
      end
    end
    ...
    context 'パスワードが20文字以上の場合' do
      it 'バリデーションに引っかかる' do
        user = FactoryBot.build(:user, password: 'p'*21)
        expect(user).not_to be_valid
      end
    end
    ...
    context '登録済みのメールアドレスを入力した場合' do
      it "バリデーションに引っかかる" do
        user = FactoryBot.create(:user, email: 'aaa@test.com')
        repeat_user = FactoryBot.build(:user, email: 'aaa@test.com')
        expect(repeat_user).not_to be_valid
      end
    end
    context 'メールアドレスのフォーマットが不正な場合' do
      it 'バリデーションが引っかかる' do
        user = FactoryBot.build(:user, email: 'aaa.test.com')
        expect(user).not_to be_valid
      end
    end
